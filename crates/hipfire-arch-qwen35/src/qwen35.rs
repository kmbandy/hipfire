...
pub struct MoeFfnWeights {
    ...
    /// signature.
    pub pager: Option<std::rc::Rc<std::cell::RefCell<hipfire_runtime::weight_pager::WeightPager>>>,
}
...
pub fn load_weights(hfq: &mut HfqFile, config: &Qwen35Config, gpu: &mut Gpu) -> HipResult<Qwen35Weights> {
    ...
    let pager_for_loader: Option<std::rc::Rc<std::cell::RefCell<hipfire_runtime::weight_pager::WeightPager>>> =
        if config.paged_experts || config.paged_dense {
            let pager_cfg = hipfire_runtime::weight_pager::PagerConfig {
            ...
            let pager = match transport_choice.as_str() {
                ...
                "iouring" => {
                    ...
                    hipfire_runtime::weight_pager::WeightPager::with_iouring_transport(
                        hfq.path(), pager_cfg,
                    ).map_err(|e| hip_bridge::HipError::new(0, &format!(
                ...
                "pread" => {
                    ...
                    hipfire_runtime::weight_pager::WeightPager::with_pread_transport(
                        hfq.path(), pager_cfg,
                    ).map_err(|e| hip_bridge::HipError::new(0, &format!(
...
fn load_dense_ffn(
    ...
    config: &Qwen35Config,
    layer_idx: u16,
    pager: Option<&std::rc::Rc<std::cell::RefCell<hipfire_runtime::weight_pager::WeightPager>>>,
    ...
    // ─── Paged path ───────────────────────────────────────────────────
    let resolve = |bare: &str| -> Option<&hipfire_runtime::hfq::HfqTensorInfo> {
    ...
    {
        use hipfire_runtime::weight_pager::{ByteRange, FfnRole, WeightId};
...
fn pager_fill_dense_ffn_layer(
    ...
    use hipfire_runtime::weight_pager::{FfnRole, WeightId};
    let mut pager = pager_rc.borrow_mut();
    pager.fill_into(
        WeightId::DenseFfn { layer: layer_idx, role: FfnRole::Gate },
...
fn pager_dense_async_prefetch_layer(
    ...
    slot: usize,
    gpu: &mut Gpu,
) -> HipResult<Vec<hipfire_runtime::weight_pager::TransferEventIdx>> {
    let (Some(scratch), Some(pager_rc)) = (&weights.dense_scratch, &weights.pager) else {
        return Ok(Vec::new());
    };
    use hipfire_runtime::weight_pager::{ByteRange, FfnRole, WeightId};
...
fn load_moe_ffn(
    ...
    let (experts, expert_shape, expert_gate_up_ptrs, expert_down_ptrs) = match pager {
        ...
        Some(pager_rc) => {
            ...
            use hipfire_runtime::weight_pager::{ByteRange, ExpertRole, ExpertShape, WeightId};
...
fn qwen35_paged_moe_dispatch(
    gpu: &mut Gpu,
    ffn: &MoeFfnWeights,
    pager_rc: &std::rc::Rc<std::cell::RefCell<hipfire_runtime::weight_pager::WeightPager>>,
    topk_indices: &[usize],
    topk_weights: &[f32],
    xr: &GpuTensor,
    x_residual: &GpuTensor,
    ...
    mi: usize,
    k: usize,
    shape: &hipfire_runtime::weight_pager::ExpertShape,
) -> HipResult<()> {
    use hipfire_runtime::weight_pager::{ExpertRole, WeightId};
    ...
    // overlap to show for it).
    {
        ...
        if pager.host_arena_pinned() {
            ...
            // pays off.
            let batch_input: Vec<(WeightId, hipfire_runtime::weight_pager::ByteRange)> = topk_indices
                .iter()
                .flat_map(|&idx| {
...
fn forward_scratch_layers(
    ...
    let mut pending_prefetch: Option<Vec<hipfire_runtime::weight_pager::TransferEventIdx>> = None;
    ...
    for layer_idx in 0..config.n_layers {
        ...
        match (&weights.layers[layer_idx], config.layer_types[layer_idx]) {
            (LayerWeights::DeltaNet(layer), LayerType::LinearAttention) => {
                ...
                // dt_g reads `eff_w_*` so paging substitutes the right tensor;
                // upstream's lloyd body uses `layer.w_*` directly — combining
                // lloyd-MQ3 with paged_dense is therefore a follow-up.
                let dt_g = eff_w_gate.gpu_dtype;
                let same_dtype = eff_w_up.gpu_dtype == dt_g;
                let fused_gu_mq4 = same_dtype && (dt_g == DType::MQ4G256 || dt_g == DType::HFQ4G256);
                let fused_gu_lloyd_mq3 = same_dtype && dt_g == DType::MQ3G256Lloyd;
                // Phase A.1c (gfx906): fused dp4a path for HFQ6/MQ6 weights.
                let fused_gu_hfq6 = same_dtype
            ...
            (LayerWeights::FullAttn(layer), LayerType::FullAttention) => {
                ...
                // Cross-arch fast path: fused gate+up in one launch. Works
                // for both MQ4 (x_rot Some) and HF4 (x_rot None → s.tmp).
                // See dense arm above for the lloyd-MQ3 + paged_dense caveat.
                let dt_g = eff_w_gate.gpu_dtype;
                let same_dtype = eff_w_up.gpu_dtype == dt_g;
                let fused_gu_mq4 = same_dtype && (dt_g == DType::MQ4G256 || dt_g == DType::HFQ4G256);
                let fused_gu_lloyd_mq3 = same_dtype && dt_g == DType::MQ3G256Lloyd;
